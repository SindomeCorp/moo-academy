...
const children = (n) => n.children.filter(c => c.named);
function field(n, name) { return n.children.find(c => c.field === name); }
function reject(n, message, category = 'unsupported-feature') {
    throw new Rejection({ category, message, span: n.span });
}
export function lower(syntax, profile) {
    let depth = 0, dollarDepth = 0;
    const loops = [];
    function expression(n) {
        if (++depth > 200)
            reject(n, 'Expression nesting exceeds the supported compile depth');
        try {
            ...
            switch (n.kind) {
                ...
                case 'object_reference': {
                    ...
                    if (text.length > 20)
                        reject(n, 'Integer literal outside profile range', 'syntax-error');
                ...
                case 'float': {
                    ...
                    if (!Number.isFinite(value))
                        reject(n, 'Float literal must be finite', 'syntax-error');
                    return { kind: 'literal', value: moo.float(value), span };
                }
                case 'string': {
                    const value = n.text.slice(1, -1).replace(/\\(.)/gs, '$1');
                    if (/[^\x09\x20-\x7e]/.test(value))
                        reject(n, 'Only printable ASCII and tab string characters are currently supported');
                    return { kind: 'literal', value: moo.string(value), span };
                }
                case 'error_constant':
                    if (profile === 'lambdamoo' && errorCodes.indexOf(n.text) > 15)
                        reject(n, `${n.text} requires ToastStunt`);
                    if (!errorCodes.includes(n.text))
                        reject(n, `Unknown error constant ${n.text}`, 'syntax-error');
                    return { kind: 'literal', value: moo.error(n.text), span };
                case 'type_constant': {
                    if (profile === 'toaststunt' && ['WAIF', 'ANON'].includes(n.text))
                        return { kind: 'literal', value: moo.int(n.text === 'WAIF' ? 13 : 12), span };
                    const tags = { INT: 'int', NUM: 'int', OBJ: 'object', STR: 'string', ERR: 'error', LIST: 'list', FLOAT: 'float', MAP: 'map' };
                    const tag = tags[n.text];
                    if (!tag || (tag === 'map' && profile === 'lambdamoo'))
                        reject(n, `${n.text} type is unsupported in ${profile}`);
                    return { kind: 'variable', name: fold(n.text), span };
                }
                case 'identifier': return { kind: 'variable', name: fold(n.text), span };
                case 'dollar':
                    if (!dollarDepth)
                        reject(n, '$ requires enclosing index or slice brackets', 'syntax-error');
                    return { kind: 'dollar', span };
                ...
                case 'unary_expression': {
                    ...
                    if (!['-', '!'].includes(operator))
                        reject(n, `Unary ${operator} is not supported`);
                ...
                case 'binary_expression': {
                    ...
                    if (!['+', '-', '*', '/', '%', '^', '==', '!=', '<', '<=', '>', '>=', 'in', '&&', '||'].includes(operator))
                        reject(n, `Binary ${operator || 'not in'} is not supported`);
                ...
                case 'assignment_statement': {
                    const operator = n.children.find(c => ['=', '+=', '-='].includes(c.text))?.text;
                    if (!operator)
                        reject(n, 'Missing assignment operator', 'syntax-error');
                    const left = field(n, 'left');
                    if (left.kind === 'scatter_pattern') {
                        if (operator !== '=')
                            reject(n, 'Scatter assignment requires =', 'syntax-error');
                        const entries = children(left).map(entry => {
                            if (entry.kind === 'identifier')
                                return { name: fold(entry.text), mode: 'required', fallback: null };
                        ...
                        if (entries.filter(e => e.mode === 'rest').length > 1)
                            reject(left, 'Scatter permits at most one rest target', 'syntax-error');
                ...
                case 'range_expression': {
                    ...
                    try {
                        if (n.kind === 'index_expression')
                            return { kind: 'index', collection, index: expression(field(n, 'index')), span };
                ...
                case 'function_call': {
                    ...
                    if (name.kind === 'system_object')
                        return { kind: 'verb-call', receiver: { kind: 'literal', value: moo.object(0), span }, name: { kind: 'literal', value: moo.string(name.text.slice(1)), span }, arguments: children(field(n, 'arguments')).map(argument), span };
    ...
    function target(n) {
        ...
        if (!validate(result))
            reject(n, 'Assignment requires a variable or indexed/sliced variable', 'syntax-error');
        return result;
    }
    function argument(n) {
    ...
    function codes(nodes) {
        if (nodes.length === 1 && nodes[0].kind === 'identifier' && nodes[0].text.toUpperCase() === 'ANY')
            return 'any';
        return nodes.map(argument);
    }
    function statement(n) {
        if (++depth > 200)
            reject(n, 'Statement nesting exceeds the supported compile depth');
        try {
            ...
            switch (n.kind) {
                ...
                case 'while_statement': {
                    ...
                    loops.push(null);
                    try {
                        return { kind: 'while', condition, body: named.filter(c => c !== cond).map(statement), span };
                    }
                    finally {
                        loops.pop();
                    }
                ...
                case 'for_statement': {
                    ...
                    if (key && profile !== 'toaststunt')
                        reject(n, 'Two-variable iteration requires ToastStunt');
                    const name = field(n, 'value'), source = field(n, 'collection');
                    const variable = fold(name.text);
                    let collection;
                    if (source.kind === 'range_literal') {
                        if (!field(source, 'start') || !field(source, 'end'))
                            reject(source, 'Range loops require both bounds', 'syntax-error');
                    ...
                    loops.push(variable);
                    try {
                        return { kind: 'for', variable, ...(key ? { key: fold(key.text) } : {}), collection, body: named.filter(c => c !== name && c !== source && c !== key).map(statement), span };
                    }
                    finally {
                        loops.pop();
                    }
                ...
                case 'continue_statement': {
                    ...
                    if (!loops.length || (label && !loops.includes(label)))
                        reject(n, 'Loop control must target an enclosing loop', 'syntax-error');
                    return { kind: n.kind === 'break_statement' ? 'break' : 'continue', label, span };
                }
                case 'try_statement': {
                    const clauses = named.filter(c => c.kind === 'except_clause');
                    if (clauses.length > 255)
                        reject(n, 'At most 255 except clauses are supported', 'syntax-error');
                    const final = named.find(c => c.kind === 'finally_clause');
                    const handlers = clauses.map(clause => {
                        if (field(clause, 'handler_value'))
                            reject(clause, 'Except => handlers are outside the supported syntax');
                        const variable = field(clause, 'variable');
                        const codeNodes = [], bodyNodes = [];
                        let inBody = false;
                        for (const child of children(clause)) {
                            if (child === variable)
                                continue;
                            if (child.kind.endsWith('_statement') || child.kind === 'comment')
                                inBody = true;
    ...
    try {
        return { ok: true, body: children(syntax).map(statement) };
    }
    catch (error) {
