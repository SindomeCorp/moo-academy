import { validateEnvironment } from '../host/environment.js';
...
function limitsFor(limits = {}) {
    ...
    for (const value of Object.values(result))
        if (!Number.isSafeInteger(value) || value < 0)
            throw new HostError('Snapshot limits must be nonnegative safe integers');
    if (result.maxDepth > 1024)
        throw new HostError('Snapshot maxDepth cannot exceed 1024');
    return result;
}
function reject(message) { throw new HostError('Invalid world snapshot: ' + message); }
/** Inspect plain data without reading accessors or following cycles. */
function checkJSON(input, limits) {
    ...
    while (pending.length) {
        ...
        if (++nodes > limits.maxNodes || depth > limits.maxDepth)
            reject('JSON depth/node limit exceeded');
        if (typeof value === 'string') {
            characters += value.length;
            if (characters > limits.maxCharacters)
                reject('JSON string limit exceeded');
            continue;
        }
        ...
        if (!value || typeof value !== 'object')
            reject('only JSON data is accepted');
        if (seen.has(value))
            reject('cyclic or shared object structure');
        seen.add(value);
        const array = Array.isArray(value);
        if ((!array && Object.getPrototypeOf(value) !== Object.prototype) || (array && Object.getPrototypeOf(value) !== Array.prototype))
            reject('custom object prototypes are not accepted');
        if (array && value.length > limits.maxNodes - nodes)
            reject('array exceeds node limit');
        const keys = Reflect.ownKeys(value);
        if (keys.length > limits.maxNodes - nodes + (array ? 1 : 0))
            reject('object exceeds node limit');
        if (array && keys.length !== value.length + 1)
            reject('sparse or extended arrays are not accepted');
        for (const key of keys) {
            if (array && key === 'length')
                continue;
            if (typeof key !== 'string')
                reject('symbol keys are not accepted');
            const descriptor = Object.getOwnPropertyDescriptor(value, key);
            if (!Object.hasOwn(descriptor, 'value') || !descriptor.enumerable)
                reject('accessor or hidden fields are not accepted');
...
function array(value) { if (!Array.isArray(value))
    reject('expected an array'); return value; }
function string(value) { if (typeof value !== 'string')
    reject('expected a string'); return value; }
function decimal(value) {
    const text = string(value);
    if (text.length > 20 || !/^(0|-?[1-9][0-9]*)$/.test(text))
        reject('expected a canonical decimal ID');
    return BigInt(text);
}
function envelope(input, limits) {
    const bounds = limitsFor(limits);
    let parsed = input;
    if (typeof input === 'string') {
        if (input.length > bounds.maxCharacters)
            reject('serialized character limit exceeded');
        try {
            parsed = JSON.parse(input);
        }
        catch (cause) {
            throw new HostError('Invalid snapshot JSON', { cause });
        }
    }
    checkJSON(parsed, bounds);
    const data = record(parsed, ['version', 'profile', 'nextId', 'objects'], ['environment']);
    if (data.version !== 1)
        reject('unsupported or missing snapshot version');
    assertProfile(data.profile);
    return data;
}
export function snapshotProfile(input, limits) { return envelope(input, limits).profile; }
export function worldSnapshot(world, limits) {
    world.assertIdle();
    const snapshot = { ...(world.environment ? { environment: structuredClone(world.environment) } : {}), version: 1, profile: world.profile, nextId: String(world.nextId), objects: world.objects().map(object => ({
...
export function saveWorld(world, limits) {
    const snapshot = worldSnapshot(world, limits);
    const json = JSON.stringify(snapshot);
    if (json.length > limitsFor(limits).maxCharacters)
        reject('serialized character limit exceeded');
    return json;
}
export function decodeWorld(input, compiler, options = {}) {
    if (options.unsupportedSourcePolicy !== undefined && !['reject', 'retain'].includes(options.unsupportedSourcePolicy))
        reject('invalid unsupported source policy');
    options.world?.assertIdle();
    const data = envelope(input, options.limits), profile = data.profile;
    if (profile !== compiler.profile || (options.world && options.world.profile !== profile))
        reject('snapshot, runtime and target profiles must match');
    const world = new World({ profile, limits: options.world?.limits ?? options.worldLimits ?? {} });
    const rawObjects = array(data.objects);
    if (rawObjects.length > world.limits.objects)
    ...
    const objects = rawObjects.map(raw => {
        ...
        if (object.player !== undefined && typeof object.player !== 'boolean')
            reject('player must be boolean');
        const flags = record(object.flags, ['programmer', 'wizard', 'r', 'w', 'f']);
        for (const flag of Object.values(flags))
            if (flag !== 0 && flag !== 1)
                reject('flags must be zero or one');
        const rawProperties = array(object.properties), rawVerbs = array(object.verbs);
        properties += rawProperties.length;
        verbs += rawVerbs.length;
        if (properties > world.limits.properties || verbs > world.limits.verbs)
            reject('property/verb limit exceeded');
        const propertyRecords = rawProperties.map(rawProperty => {
        ...
        const verbRecords = rawVerbs.map(rawVerb => {
            const verb = record(rawVerb, ['names', 'owner', 'perms', 'args'], ['source', 'hostId']);
            if (Object.hasOwn(verb, 'source') === Object.hasOwn(verb, 'hostId'))
                reject('verb requires exactly one of source or hostId');
            const args = array(verb.args);
            if (args.length !== 3)
                reject('verb arguments require three strings');
        ...
        return { id: decimal(object.id), parent: decimal(object.parent), owner: decimal(object.owner), name: string(object.name),
            ...
            flags: flags, properties: propertyRecords, verbs: verbRecords };
    });
    ...
    for (const object of world.objects())
        for (const verb of object.verbs) {
            if (verb.hostId !== undefined) {
                if (!compiler.hasHostVerb(verb.hostId))
                    reject('host verb registration is unavailable: ' + verb.hostId);
            }
            else {
                const result = compiler.compile(verb.source);
                if (!result.ok && !(options.unsupportedSourcePolicy === 'retain' && result.diagnostics.every(d => d.category === 'unsupported-feature')))
                    reject('verb #' + object.id + ':' + verb.names + ' does not compile: ' + result.diagnostics.map(diagnostic => diagnostic.message).join('; '));
            }
        }
    if (options.world) {
        options.world.replaceWith(world);
