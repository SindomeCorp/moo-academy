import { moo, errorCodes } from './index.js';
import { fail } from '../runtime/errors.js';
export const typeNumbers = { int: 0, object: 1, string: 2, error: 3, list: 4, float: 9, map: 10 };
export const fold = (s) => s.replace(/[A-Z]/g, c => c.toLowerCase());
export function truth(v) {
    switch (v.type) {
        ...
        case 'string':
        case 'list':
        case 'map': return v.value.length !== 0;
        default: return false;
    }
}
export function equal(a, b, budget, caseSensitive = false) {
    const pending = [[a, b]];
    while (pending.length) {
        ...
        if (x.type === 'string' && y.type === 'string') {
            ...
            if ((caseSensitive ? x.value : fold(x.value)) !== (caseSensitive ? y.value : fold(y.value)))
                return false;
        }
...
export function compare(a, b, budget, mapKey = false) {
    budget.step();
    if (a.type === 'list' || a.type === 'map' || b.type === 'list' || b.type === 'map')
        fail('E_TYPE');
    if (a.type !== b.type) {
        if (!mapKey)
            fail('E_TYPE');
        // ToastStunt's comparison uses internal type codes, including the string flag.
        const code = (v) => v.type === 'string' ? 130 : typeNumbers[v.type];
        return code(a) - code(b);
    }
    let left = a.value, right = b.value;
    if (a.type === 'string' && b.type === 'string') {
        ...
        left = errorCodes.indexOf(a.value);
        right = errorCodes.indexOf(b.value);
    }
    return left < right ? -1 : left > right ? 1 : 0;
}
export function wrap(value, profile) { return moo.int(BigInt.asIntN(profile === 'lambdamoo' ? 32 : 64, value)); }
export function unary(op, v, profile) {
    if (op === '!')
        return moo.int(truth(v) ? 0 : 1);
    if (v.type !== 'int' && v.type !== 'float')
        fail('E_TYPE');
    if (op === '+')
        return v;
    if (op === '-')
        return v.type === 'int' ? wrap(-v.value, profile) : moo.float(-v.value);
    if (op === '~' && v.type === 'int')
        return wrap(~v.value, profile);
    return fail('E_TYPE');
}
export function binary(op, a, b, profile, budget) {
    if (op === '==' || op === '!=')
        return moo.int(equal(a, b, budget) === (op === '==') ? 1 : 0);
    if (['<', '<=', '>', '>='].includes(op)) {
    ...
    if (op === 'in') {
        if (b.type === 'list' || (profile === 'toaststunt' && b.type === 'map')) {
            for (let i = 0; i < b.value.length; i++)
                if (equal(a, b.type === 'list' ? b.value[i] : b.value[i][1], budget))
                    return moo.int(i + 1);
            return moo.int(0);
        }
    ...
    if (op === '+' && a.type === 'string' && b.type === 'string') {
        budget.allocate(a.value.length + b.value.length);
        return moo.string(a.value + b.value);
    }
    if (a.type === 'int' && b.type === 'int') {
        ...
        switch (op) {
            ...
            case '%':
                if (y === 0n)
                    fail('E_DIV');
                result = profile === 'toaststunt' ? ((x % y) + y) % y : x % y;
                break;
            case '^': {
                if (y < 0n) {
                    if (x === 0n)
                    ...
                    let base = x, exponent = y;
                    result = 1n;
                    while (exponent) {
                        budget.step();
                        if (exponent & 1n)
                            result = BigInt.asIntN(bits, result * base);
                        base = BigInt.asIntN(bits, base * base);
                        exponent >>= 1n;
                    }
                }
                break;
    ...
    if (a.type === 'float' && (b.type === 'float' || (op === '^' && b.type === 'int'))) {
        ...
        if (Number.isNaN(result))
            fail(profile === 'lambdamoo' ? 'E_INVARG' : 'E_FLOAT');
...
export function length(value) {
    if (value.type !== 'list' && value.type !== 'string' && value.type !== 'map')
        fail('E_TYPE');
...
function integerIndex(value) { if (value.type !== 'int')
    fail('E_TYPE'); return value.value; }
export function index(value, key, budget) {
    if (value.type === 'map') {
        if (key.type === 'list' || key.type === 'map')
            fail('E_TYPE');
        for (const [k, v] of value.value)
            if (compare(k, key, budget, true) === 0)
                return v;
        return fail('E_RANGE');
    }
    sequence(value);
    const at = integerIndex(key);
    if (at < 1n || at > BigInt(value.value.length))
        fail('E_RANGE');
    return value.type === 'string' ? moo.string(value.value[Number(at) - 1]) : value.value[Number(at) - 1];
}
export function slice(value, start, end, budget) {
    sequence(value);
    const low = integerIndex(start), high = integerIndex(end);
    if (low > high)
        return value.type === 'list' ? moo.list([]) : moo.string('');
    if (low < 1n || high > BigInt(value.value.length))
        fail('E_RANGE');
...
export function mapSet(map, key, value, budget) {
    if (map.type !== 'map' || key.type === 'list' || key.type === 'map')
        fail('E_TYPE');
    budget.allocate(map.value.length * 2 + 2);
    const entries = map.value.filter(([k]) => compare(k, key, budget, true) !== 0);
    entries.push([key, value]);
    entries.sort(([a], [b]) => compare(a, b, budget, true));
    return moo.map(entries);
}
export function replaceIndex(value, key, replacement, budget) {
    if (value.type === 'map')
        return mapSet(value, key, replacement, budget);
    sequence(value);
    const at = integerIndex(key);
    if (at < 1n || at > BigInt(value.value.length))
        fail('E_RANGE');
