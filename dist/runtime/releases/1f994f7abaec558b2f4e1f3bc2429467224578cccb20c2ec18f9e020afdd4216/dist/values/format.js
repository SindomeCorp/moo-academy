...
export function format(value, literal, budget) {
    const parts = [];
    const pending = [value];
    while (pending.length) {
        ...
        if (typeof current === 'string') {
            budget.allocate(current.length);
            parts.push(current);
            continue;
        }
        switch (current.type) {
            case 'int':
                pending.push(String(current.value));
                break;
            case 'object':
                pending.push('#' + current.value);
                break;
            case 'float':
                pending.push(floatText(current.value));
                break;
            case 'error':
                pending.push(literal ? current.value : errorMessages[current.value]);
                break;
            case 'string':
            ...
            case 'map':
                ...
                pending.push(']');
                for (let i = current.value.length - 1; i >= 0; i--) {
                    pending.push(current.value[i][1], ' -> ', current.value[i][0]);
                    if (i)
                        pending.push(', ');
                }
                pending.push('[');
