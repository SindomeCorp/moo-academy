...
import { fail, MooError, LimitError, UnsupportedSourceError } from './errors.js';
...
import { HostRaisedError, raiseFromHost } from './registrations.js';
export class Execution {
    ...
    taskLocal = moo.int(0);
    #cache = new WeakMap();
    constructor(world, budget, compile, runId, onOutput, hostVerbs = new Map(), taskId = 1) {
        this.world = world;
        this.budget = budget;
        this.compile = compile;
        this.runId = runId;
        this.onOutput = onOutput;
        this.hostVerbs = hostVerbs;
        this.taskId = taskId;
    }
    get current() { return this.frames.at(-1); }
    *run(body, frame, args) {
        this.budget.call(this.frames.length + 1);
        this.frames.push(frame);
        const evaluator = new Evaluator(this.world.profile, this.budget, this);
        for (const key of ['this', 'player', 'caller'])
            evaluator.variables.set(key, moo.object(frame[key]));
        evaluator.variables.set('args', moo.list(args));
        evaluator.variables.set('verb', moo.string(frame.verb));
        try {
            return (yield* evaluator.evaluate(body));
        }
        catch (error) {
    ...
    *pass(args, span) {
        this.current.span = span;
        if (this.current.definer === -1n)
            fail('E_VERBNF', 'pass() requires a defined verb frame');
    ...
    *invoke(receiver, name, args, start) {
        const found = start === -1n ? undefined : this.world.findVerb(start, name, this.budget);
        if (!found)
            fail('E_VERBNF', `World verb #${receiver}:${name} does not exist`);
        const frame = { this: receiver, definer: found.definer, player: this.current.player,
        ...
        if (found.verb.hostId !== undefined) {
            ...
            if (!implementation)
                throw new HostError('Host verb registration ' + found.verb.hostId + ' is unavailable');
            this.budget.call(this.frames.length + 1);
            this.frames.push(frame);
            try {
                const result = implementation(Object.freeze({
                    world: this.world, frame: Object.freeze({ ...frame }), args: Object.freeze([...args]),
                    notify: (recipient, text) => { this.notify(recipient, moo.string(text)); },
                    chargeSteps: (count) => this.budget.step(count),
                    chargeAllocations: (count) => this.budget.allocate(count),
                    raise: (code, message, value) => {
                        if (value !== undefined)
                            encodeValue(value, { profile: this.world.profile });
                ...
                if (error instanceof HostRaisedError || error instanceof LimitError) {
                    if (!error.frames)
                        error.frames = this.frames.slice().reverse().map(f => ({ ...f }));
                    throw error;
                }
        ...
        if (!body) {
            ...
            if (!compiled.ok && compiled.diagnostics.every(d => d.category === 'unsupported-feature'))
                throw new UnsupportedSourceError(compiled.diagnostics.map(d => ({ ...d, message: `Stored verb #${found.definer}:${name}: ${d.message}` })));
            if (!compiled.ok)
                throw new HostError(`Stored verb #${found.definer}:${name} does not compile: ${compiled.diagnostics.map(d => d.message).join('; ')}`);
            body = compiled.body;
            this.#cache.set(found.verb, body);
        }
        return (yield* this.run(body, frame, args));
    }
    *builtin(name, args, span) {
        this.current.span = span;
        return (yield* invokeWorldBuiltin(name, args, this));
    }
    *initialize(id) { if (this.world.findVerb(id, 'initialize', this.budget))
        (yield* this.invoke(id, 'initialize', [], id)); }
    *move(id, destination, position) {
        const object = this.world.get(id);
        if (destination !== -1n)
            this.world.get(destination);
        if (destination !== -1n && destination !== (object.location ?? -1n)) {
            ...
            if (!truth(accepts) && !wizard)
                fail('E_NACC');
        }
        if (!this.world.valid(id) || (destination !== -1n && !this.world.valid(destination)))
            return moo.int(0);
        const old = this.world.get(id).location ?? -1n;
        this.world.relocate(id, destination, position, this.budget);
        if (old !== destination) {
            if (this.world.valid(old) && this.world.findVerb(old, 'exitfunc', this.budget))
                (yield* this.invoke(old, 'exitfunc', [moo.object(id)], old));
            if (this.world.valid(id) && this.world.valid(destination) && this.world.get(id).location === destination && this.world.findVerb(destination, 'enterfunc', this.budget))
                (yield* this.invoke(destination, 'enterfunc', [moo.object(id)], destination));
        }
        return moo.int(0);
    }
    *dynamicBuiltin(name, args) {
        name = name.replace(/[A-Z]/g, c => c.toLowerCase());
        this.budget.enter();
        try {
            this.budget.step();
            checkBuiltinArity(name, args.length, this.world.profile);
            if (name === 'pass') {
                if (this.current.definer === -1n)
                    fail('E_VERBNF');
    ...
    *evaluateSource(lines) {
        const length = lines.reduce((n, s) => n + s.length + 1, 0);
        this.budget.step(length);
        this.budget.allocate(length + 2);
        const compiled = this.compile(lines.join('\n'));
        if (!compiled.ok) {
            this.budget.allocate(compiled.diagnostics.reduce((n, d) => n + d.message.length + 1, 0));
            return moo.list([moo.int(0), moo.list(compiled.diagnostics.map(d => moo.string(d.message)))]);
        }
    ...
    notify(recipient, message) {
        if (recipient.type !== 'object' || message.type !== 'string')
            fail('E_TYPE');
        ...
        if (this.world.environment?.connections?.some(c => c.player === String(recipient.value))) {
            ...
            conn.output.push(message.value);
            state.time++;
            this.world.setEnvironment(state, this.budget);
        }
        ...
        try {
            ...
            throw new HostError('Output delivery failed', { cause });
        }
        return moo.int(1);
    }
    traceback(error) {
