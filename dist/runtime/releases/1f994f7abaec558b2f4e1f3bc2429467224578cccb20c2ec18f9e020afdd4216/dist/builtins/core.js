...
const registry = new Map([
    ...[...utilities, ...codecs, ...regexBuiltins, ...cryptoBuiltins, ...clockBuiltins].map(utility => [utility.name, { invoke: utility.invoke }]),
    ...
    ...['index', 'rindex'].map((name) => [name, { invoke: ([source, needle, caseMatters, offset], { profile, budget }) => {
                if (offset && profile === 'lambdamoo')
                    fail('E_ARGS');
                if (source.type !== 'string' || needle.type !== 'string' || (offset && offset.type !== 'int'))
                    fail('E_TYPE');
                const shift = offset?.value ?? 0n;
                const reverse = name === 'rindex';
                if ((reverse && shift > 0n) || (!reverse && shift < 0n))
                    fail('E_INVARG');
                // Never reproduce the reference's out-of-bounds C pointer arithmetic.
                if (shift > BigInt(source.value.length) || shift < -BigInt(source.value.length))
                    fail('E_INVARG');
                ...
                for (let at = reverse ? end : start; at >= start && at <= end; at += reverse ? -1 : 1) {
                    ...
                    for (let i = 0; i < needle.value.length; i++) {
                        budget.step();
                        if (char(source.value, at + i) !== char(needle.value, i)) {
                            matches = false;
                            break;
                        }
                    }
                    if (matches)
                        return moo.int(at - start + 1);
                }
                return moo.int(0);
            },
        }]),
    ...
    ['raise', { invoke: ([code, message, value]) => {
                if (code.type !== 'error' || (message && message.type !== 'string'))
                    fail('E_TYPE');
                throw new MooError(code.value, message?.value, value);
            } }],
    ...['mapkeys', 'mapvalues'].map((name) => [name, {
            invoke: ([value, ...keys], { profile, budget }) => {
                if (profile !== 'toaststunt')
                    fail('E_INVARG', `Host built-in ${name} is unavailable in ${profile}`);
                if (value.type !== 'map')
                    fail('E_TYPE');
                if (keys.length) {
    ...
    ['maphaskey', { invoke: ([map, key, caseMatters], { profile, budget }) => {
                if (profile !== 'toaststunt')
                    fail('E_INVARG', 'Host built-in maphaskey is unavailable in lambdamoo');
                if (map.type !== 'map' || key.type === 'list' || key.type === 'map' || (caseMatters && caseMatters.type !== 'int'))
                    fail('E_TYPE');
    ...
    ['mapdelete', { invoke: ([map, key], { profile, budget }) => {
                if (profile !== 'toaststunt')
                    fail('E_INVARG', 'Host built-in mapdelete is unavailable in lambdamoo');
                if (map.type !== 'map')
                    fail('E_TYPE');
                ...
                for (const keyValue of keys) {
                    if (keyValue.type === 'list' || keyValue.type === 'map')
                        fail('E_TYPE');
                    const at = entries.findIndex(([candidate]) => compare(candidate, keyValue, budget, true) === 0);
                    if (at < 0) {
                        if (key.type === 'list')
                            throw new MooError('E_RANGE', `Key ${format(keyValue, true, budget)} not found in map`, keyValue);
                        fail('E_RANGE');
                    }
...
export function invokeBuiltin(name, args, context) {
    const builtin = registry.get(name);
    if (!builtin)
        fail('E_INVARG', `Host built-in ${name} is not provided by this runtime`);
