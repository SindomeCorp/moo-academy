...
export const connectionBuiltins = specs.map(([name, parameters, returns, profile]) => ({ name, parameters, returns, ...(profile ? { profiles: [profile] } : {}), summary: 'Simulated host: ' + name.replaceAll('_', ' ') + '.', notes: [
...
export function invokeConnectionBuiltin(name, args, execution) {
    const { world, budget } = execution, original = world.environment ?? initialEnvironment();
    const length = JSON.stringify(original).length;
    budget.step(length);
    budget.allocate(length);
    const state = structuredClone(original);
    state.connections ??= [];
    const a = args[0], b = args[1], c = args[2];
    const commit = (value = moo.int(0)) => { state.time++; world.setEnvironment(state, budget); return value; };
    if (name === 'connected_players') {
        const connections = state.connections.filter(c => BigInt(c.player) >= 0n || a && truth(a));
        budget.allocate(connections.length);
        return moo.list(connections.map(c => moo.object(BigInt(c.player))));
    }
    if (name === 'server_version') {
        if (a && truth(a))
            fail('E_INVARG');
        return text('moo-in-javascript/0.2.0 (educational)', budget);
    }
    if (name === 'server_log') {
        const message = str(a);
        state.logs ??= [];
    ...
    if (name === 'shutdown') {
        if (b && truth(b))
            fail('E_INVARG', 'Native process restart is not supported');
        state.shutdown = a ? str(a) : 'Shutdown requested';
        return commit();
    }
    if (name === 'dump_database') {
        const environment = structuredClone(state);
        delete environment.checkpoint;
        const copy = new World({ profile: world.profile, limits: world.limits });
        copy.restore(world.objects(), world.nextId, environment);
        state.checkpoint = saveWorld(copy);
        return commit();
    }
    if (name === 'db_disk_size') {
        const checkpoint = state.checkpoint ?? '';
        budget.allocate(checkpoint.length * 3);
        return moo.int(new TextEncoder().encode(checkpoint).length);
    }
    if (name === 'buffered_output_length' && !a) {
        const size = state.connections.reduce((sum, c) => sum + c.output.reduce((n, s) => n + s.length, 0), 0);
        return moo.int(size);
    }
    const id = a ? obj(a) : execution.current.player;
    let conn = state.connections.find(c => c.player === String(id));
    if (name === 'force_input' && !conn) {
        world.get(id);
        conn = virtualConnection(String(id), state.time);
        state.connections.push(conn);
    }
    if (!conn && name === 'boot_player')
        return moo.int(0);
    if (!conn)
        fail('E_INVARG', 'No virtual connection for this object');
    switch (name) {
        ...
        case 'connection_info':
            ...
            ].map(([key, value]) => [moo.string(key), value]));
        case 'connection_option':
        case 'connection_options': {
            if (b) {
                const key = fold(str(b));
                if (!Object.hasOwn(conn.options, key))
                    fail('E_INVARG');
                return moo.int(conn.options[key] ? 1 : 0);
            }
            budget.allocate(Object.keys(conn.options).length * 3);
            return moo.list(Object.entries(conn.options).map(([key, value]) => moo.list([moo.string(key), moo.int(value ? 1 : 0)])));
        }
        case 'set_connection_option': {
            const key = fold(str(b));
            if (!Object.hasOwn(conn.options, key))
                fail('E_INVARG');
            conn.options[key] = truth(c);
            return commit();
        }
        case 'boot_player':
        ...
        case 'output_delimiters':
            budget.allocate(2);
            return moo.list([moo.string(conn.prefix), moo.string(conn.suffix)]);
        case 'force_input':
            if (c && truth(c))
                conn.input.unshift(str(b));
            else
                conn.input.push(str(b));
            return commit();
        case 'flush_input':
            conn.input = [];
            return commit();
        case 'switch_player': {
            ...
            if (c)
                int(c);
            if (state.connections.some(c => c !== conn && c.player === String(target)))
                fail('E_INVARG');
            conn.player = String(target);
            return commit();
        }
        case 'read': {
            const line = conn.input.shift();
            if (line === undefined) {
                if (b && truth(b))
                    return moo.int(0);
                return fail('E_INVARG', 'No queued input; blocking reads require a scheduler');
            }
            conn.lastActive = state.time;
            return commit(text(line, budget));
        }
    }
