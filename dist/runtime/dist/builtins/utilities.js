...
const p = (name, types, optional = false) => ({ name, types: Array.isArray(types) ? types : [types], description: name + '.', optional });
const number = ['int', 'float'];
export const utilities = [];
function add(name, parameters, returns, summary, invoke, toast = false, notes = [], rest) {
    utilities.push({ name, parameters, returns, summary, invoke, notes, ...(toast ? { profiles: ['toaststunt'] } : {}), ...(rest ? { rest } : {}) });
}
export function str(v) { if (v.type !== 'string')
    fail('E_TYPE'); return v.value; }
export function int(v) { if (v.type !== 'int')
    fail('E_TYPE'); return v.value; }
export function flt(v) { if (v.type !== 'float')
    fail('E_TYPE'); return v.value; }
export function lst(v) { if (v.type !== 'list')
    fail('E_TYPE'); return v.value; }
export function numeric(v) { if (v.type !== 'int' && v.type !== 'float')
    fail('E_TYPE'); return Number(v.value); }
export function finite(n) { if (Number.isNaN(n))
    fail('E_INVARG'); if (!Number.isFinite(n))
    fail('E_FLOAT'); return moo.float(n); }
export function text(s, budget) { budget.allocate(s.length); return moo.string(s); }
function numericValue(v, profile, object = false) {
    let n;
    switch (v.type) {
        case 'int':
        case 'object':
            ...
            break;
        case 'error':
            n = BigInt(errorCodes.indexOf(v.value));
            break;
        case 'float':
            n = BigInt(Math.trunc(v.value));
            break;
        case 'string': {
...
add('tofloat', [p('value', 'any')], 'float', 'Convert a scalar to a float.', ([v], { profile }) => {
    ...
    if (v.type === 'error')
        return moo.float(errorCodes.indexOf(v.value));
    if (v.type === 'object')
        return finite(Number(v.value));
    return finite(numeric(v));
});
for (const name of ['min', 'max'])
    add(name, [p('first', number)], 'any', `Return the ${name === 'min' ? 'smallest' : 'largest'} numeric argument.`, (args, { budget }) => {
        let best = args[0];
        numeric(best);
        for (const v of args.slice(1)) {
            ...
            if (v.type !== best.type)
                fail('E_TYPE');
            if (name === 'min' ? v.value < best.value : v.value > best.value)
                best = v;
        }
        return best;
...
add('floatstr', [p('value', 'float'), p('precision', 'int'), p('scientific', 'any', true)], 'string', 'Format a float with decimal precision.', ([a, b, c], { budget }) => {
    const n = flt(a), precision = int(b);
    if (precision < 0n)
        fail('E_INVARG');
    const digits = Number(precision > 20n ? 20n : precision);
    let out = c && truth(c) ? n.toExponential(digits) : n.toFixed(digits);
    out = out.replace(/e([+-])(\d)$/, 'e$10$2');
    return text(out, budget);
...
add('distance', [p('from', 'list'), p('to', 'list')], 'float', 'Compute Euclidean distance between coordinates.', ([a, b], { budget }) => {
    const x = lst(a), y = lst(b);
    if (x.length !== y.length)
        fail('E_INVARG');
    budget.step(x.length);
    let sum = 0;
    for (let i = 0; i < x.length; i++) {
        const d = numeric(y[i]) - numeric(x[i]);
        sum += d * d;
    }
    return finite(Math.sqrt(sum));
}, true);
add('relative_heading', [p('from', 'list'), p('to', 'list')], 'list', 'Return horizontal and vertical bearings in integer degrees.', ([a, b], { budget }) => {
    ...
    if (x.length !== 3 || y.length !== 3)
        fail('E_INVARG');
    const [dx, dy, dz] = x.map((v, i) => flt(y[i]) - flt(v));
    let angle = Math.atan2(dy, dx) * 57.2957795130823;
    if (angle < 0)
        angle += 360;
    budget.allocate(2);
...
for (const name of ['listappend', 'listinsert', 'listdelete', 'listset', 'setadd', 'setremove']) {
    ...
    add(name, params, 'list', `Return a new list after ${name}.`, ([a, b, c], { budget }) => {
        const values = lst(a);
        budget.allocate(values.length + 1);
        budget.step(values.length);
        const out = [...values];
        if (name === 'setadd' || name === 'setremove') {
            const at = values.findIndex(v => equal(v, b, budget));
            if (name === 'setadd' && at < 0)
                out.push(b);
            if (name === 'setremove' && at >= 0)
                out.splice(at, 1);
        }
        else if (name === 'listdelete' || name === 'listset') {
            const at = int(name === 'listdelete' ? b : c);
            if (at < 1n || at > BigInt(out.length))
                fail('E_RANGE');
            if (name === 'listdelete')
                out.splice(Number(at) - 1, 1);
            else
                out[Number(at) - 1] = b;
        }
        else {
            ...
            if (at < 0n)
                at = 0n;
            if (at > BigInt(out.length))
                at = BigInt(out.length);
            out.splice(Number(at), 0, b);
        }
        return moo.list(out);
    });
}
...
add('strsub', [p('source', 'string'), p('find', 'string'), p('replacement', 'string'), p('caseMatters', 'any', true)], 'string', 'Replace nonoverlapping substring occurrences.', ([a, b, c, d], { budget }) => {
    const source = str(a), needle = str(b), replacement = str(c);
    if (!needle.length)
        fail('E_INVARG');
    const hay = d && truth(d) ? source : fold(source), find = d && truth(d) ? needle : fold(needle);
    budget.step(source.length + needle.length);
    let out = '', at = 0;
    while (at < source.length) {
        budget.step();
        const next = hay.indexOf(find, at);
        if (next < 0) {
            budget.allocate(source.length - at);
            out += source.slice(at);
            break;
        }
        budget.allocate(next - at + replacement.length);
        out += source.slice(at, next) + replacement;
...
add('explode', [p('source', 'string'), p('delimiter', 'string', true), p('keepEmpty', 'int', true)], 'list', 'Split on the first delimiter character (space by default).', ([a, b, c], { budget }) => {
    const source = str(a), delimiter = b ? str(b)[0] ?? ' ' : ' ';
    if (c)
        int(c);
    budget.step(source.length);
    budget.allocate(source.length + 1);
    const parts = source.split(delimiter);
    return moo.list((c && truth(c) ? parts : parts.filter(Boolean)).map(moo.string));
}, true);
add('reverse', [p('sequence', ['list', 'string'])], 'any', 'Reverse a list or string.', ([a], { budget }) => {
    if (a.type !== 'list' && a.type !== 'string')
        fail('E_INVARG');
    budget.allocate(a.value.length);
    budget.step(a.value.length);
...
add('slice', [p('rows', 'list'), p('selector', ['int', 'list', 'string'], true), p('missingValue', 'any', true)], 'list', 'Extract selected columns or map values from a list.', ([a, b, c], { budget }) => {
    const rows = lst(a), selector = b ?? moo.int(1);
    budget.allocate(rows.length);
    const out = [];
    if (!['int', 'list', 'string'].includes(selector.type))
        fail('E_INVARG');
    if (selector.type === 'list' && (!selector.value.length || selector.value.some(v => v.type !== 'int' || v.value <= 0n)))
        fail(selector.value.some(v => v.type !== 'int') ? 'E_INVARG' : 'E_RANGE');
    if (selector.type === 'int' && selector.value <= 0n)
        fail('E_RANGE');
    for (const row of rows) {
        ...
        if (selector.type === 'string') {
            if (row.type !== 'map')
                fail('E_INVARG');
            const found = row.value.find(([key]) => equal(key, selector, budget));
            if (found)
                out.push(found[1]);
            else if (c)
                out.push(c);
            ...
            if (row.type !== 'list' && row.type !== 'string')
                fail('E_INVARG');
            if (selector.type === 'list') {
                budget.allocate(selector.value.length);
                out.push(moo.list(selector.value.map(key => index(row, key, budget))));
            }
            else
                out.push(index(row, selector, budget));
        }
    }
    return moo.list(out);
}, true);
add('sort', [p('values', 'list'), p('keys', 'list', true), p('natural', 'int', true), p('reverse', 'int', true)], 'list', 'Sort a homogeneous scalar list, optionally by separate keys.', ([a, b, c, d], { budget }) => {
    const values = lst(a), provided = b ? lst(b) : [], keys = provided.length ? provided : values;
    ...
    if (keys.some(v => v.type !== type || v.type === 'list' || v.type === 'map'))
        fail('E_TYPE');
    budget.allocate(values.length);
    const indices = keys.map((_, i) => i);
    indices.sort((i, j) => c && truth(c) && keys[i].type === 'string' ? naturalCompare(str(keys[i]), str(keys[j]), budget) : compare(keys[i], keys[j], budget));
    if (d && truth(d))
        indices.reverse();
    return moo.list(indices.map(i => values[i]));
...
function naturalCompare(left, right, budget) {
    ...
    while (i < a.length || j < b.length) {
        ...
        if (/[0-9]/.test(a[i] ?? '!') && /[0-9]/.test(b[j] ?? '!')) {
            ...
            while (/[0-9]/.test(b[j] ?? '!'))
                j++;
            const x = a.slice(ai, i), y = b.slice(bj, j);
            budget.step(x.length + y.length);
            if (x[0] === '0' || y[0] === '0') {
                if (x !== y)
                    return x < y ? -1 : 1;
...
add('simplex_noise', [p('coordinates', 'list')], 'float', 'Compute fixed-permutation simplex noise for one to four float coordinates.', ([a], { budget }) => {
    const points = lst(a).map(flt);
    if (points.length < 1 || points.length > 4)
        fail('E_TYPE');
    if (points.some(v => Math.abs(v) > 100000000))
        fail('E_INVARG');
    budget.step(256);
    return finite(simplexNoise(points));
