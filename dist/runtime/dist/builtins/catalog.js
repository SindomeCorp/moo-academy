...
const parameter = (name, types, description, optional = false) => ({ name, types: Array.isArray(types) ? types : [types], description, optional });
const result = (types, description) => ({ types: [types], description });
...
const metadata = (description) => parameter('info', 'list', description);
const value = () => parameter('value', 'any', 'MOO value.');
const nameKey = (name) => name.replace(/[A-Z]/g, c => c.toLowerCase());
...
function build(profile) {
    ...
    const add = (name, route, parameters, returns, summary, notes = [], rest) => {
        if (entries.has(name))
            throw new HostError('Duplicate builtin catalog entry: ' + name);
    ...
    for (const name of ['index', 'rindex']) {
        ...
        if (profile === 'toaststunt')
            args.push(parameter('offset', 'int', name === 'index' ? 'Nonnegative prefix length to skip; returned position is relative to the suffix.' : 'Nonpositive amount by which to shorten the search prefix.', true));
    ...
    if (profile === 'toaststunt') {
        ...
        const scalar = ['int', 'float', 'string', 'object', 'error'];
    ...
    for (const name of ['task_perms', 'caller_perms'])
        add(name, 'world', [], result('object', 'Programmer of the current or calling frame.'), 'Inspect frame programmer identity.', ['Reports identity; this educational runtime does not enforce general world permissions.']);
    if (profile === 'toaststunt') {
        ...
        for (const name of ['ancestors', 'descendants'])
            add(name, 'world', [object(), parameter('includeSelf', 'any', 'Include the queried object.', true)], result('list', 'Related objects.'), 'List ' + name + ' in the inheritance tree.');
    ...
    for (const utility of [...utilities, ...codecs, ...regexBuiltins, ...cryptoBuiltins, ...clockBuiltins, ...fileBuiltins, ...connectionBuiltins, ...sqliteBuiltins, ...serviceBuiltins]) {
        if (utility.profiles && !utility.profiles.includes(profile))
            continue;
        const parameters = profile === 'lambdamoo' && utility.name === 'listen' ? utility.parameters.map((p, i) => i === 2 ? { ...p, types: ['any'] } : p) : profile === 'lambdamoo' && ['open_network_connection', 'listeners', 'unlisten'].includes(utility.name) ? utility.parameters.slice(0, utility.name === 'listeners' ? 0 : utility.name === 'unlisten' ? 1 : 2) : profile === 'lambdamoo' && ['connection_name', 'connection_options', 'server_version', 'shutdown'].includes(utility.name) ? utility.parameters.slice(0, utility.name === 'server_version' ? 0 : 1) : profile === 'lambdamoo' && ['random', 'string_hash', 'binary_hash', 'value_hash'].includes(utility.name) ? utility.parameters.slice(0, 1) : utility.name === 'is_member' && profile === 'lambdamoo' ? utility.parameters.slice(0, 2).map(p => p.name === 'collection' ? { ...p, types: ['list'] } : p) : utility.parameters;
...
export function listBuiltins(options) {
    assertProfile(options?.profile);
    return catalogs[options.profile].list;
}
export function getBuiltinInfo(name, options) {
    assertProfile(options?.profile);
    if (typeof name !== 'string')
        throw new HostError('Builtin name must be a string');
    return catalogs[options.profile].entries.get(nameKey(name))?.info;
}
export function builtinRoute(name, profile) { return catalogs[profile].entries.get(name)?.route; }
export function checkBuiltinArity(name, count, profile) {
    const info = getBuiltinInfo(name, { profile });
    if (!info)
        fail('E_INVARG', `Host built-in ${name} is not provided by this runtime in ${profile}`);
    if (count < info.minArgs || (info.maxArgs !== null && count > info.maxArgs))
        fail('E_ARGS');
}
